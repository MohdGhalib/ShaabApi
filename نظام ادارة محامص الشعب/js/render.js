...
function _buildAuditHtml(x, isControl, isControlEmployee, isControlSub, controlEmps, auditStatusBadge) {
        ...
        if (x.audit) {
            if (isControl) {
                ...
                return `<div class="final-audit-text">
                    ...
                    ${x.auditTime ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);font-size:12px;color:var(--text-dim);">
                        ⏰ <b>${sanitize(x.auditBy||'—')}</b> — ${_toLatinDigits(x.auditTime)}
                    </div>` : ''}
    ...
    if (x.audit || _isMistreat) {
        ...
        return `<div class="final-audit-text">
            ...
            ${x.audit && x.auditTime ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);font-size:12px;color:var(--text-dim);">
                ⏰ <b>${sanitize(x.auditBy||'—')}</b> — ${_toLatinDigits(x.auditTime)}
            </div>` : ''}
            ${x.auditStatus ? `<div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border);display:flex;align-items:center;gap:10px;">
                <span style="font-size:12px;color:var(--text-dim);font-weight:700;">حالة الملاحظة</span>
